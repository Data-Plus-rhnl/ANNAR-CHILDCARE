    ...
    <>
        ...
        <div className="max-w-[1440px] mx-auto px-3.5 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-2">
            ...
            <Link href="/" className="flex items-center gap-2.5 sm:gap-3.5 group shrink-0">
              <div className="relative w-11 h-11 xs:w-12 xs:h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 transition-transform duration-300 group-hover:scale-105 shrink-0">
                <Image
              ...
              <div className="flex flex-col">
                <span className="text-[17px] xs:text-[19px] sm:text-[21px] lg:text-[22px] font-black tracking-tight text-[#2b3c6b] group-hover:text-[#830201] transition-colors leading-none">
            ...
            <div className="flex lg:hidden items-center gap-2 sm:gap-3">
              {/* Quick Uniform Pill Button (on screens >= 400px) */}
              <a
                href="https://www.cambridgeuniforms.com"
                target="_blank"
                rel="noopener noreferrer"
                className="hidden xs:flex items-center gap-1.5 bg-[#830201] hover:bg-[#6e0101] text-white px-3 py-1.5 rounded-full font-bold text-[11px] sm:text-xs uppercase shadow-sm transition-all"
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-[340px] xs:max-w-[380px] bg-white h-full shadow-2xl z-[101] flex flex-col overflow-hidden border-l border-stone-200 transition-transform duration-300">
            {/* Drawer Header */}
            <div className="p-4 sm:p-5 border-b border-stone-100 flex items-center justify-between bg-stone-50/90">
              <Link
                href="/"
                onClick={() => setMobileOpen(false)}
              ...
              >
                ...
                <div className="flex flex-col">
                  <span className="text-[17px] font-black tracking-tight text-[#2b3c6b]">
                    ANNAR <span className="text-[#830201]">CHILDCARE</span>
                  </span>
              ...
              <button
                type="button"
                onClick={() => setMobileOpen(false)}
                className="p-2 rounded-xl text-stone-600 hover:text-stone-900 bg-stone-100 hover:bg-stone-200 transition-colors"
              ...
              {navItems.map((item) => {
                ...
                if (item.dropdown) {
                  ...
                  return (
                    <div
                      key={item.name}
                      className="rounded-2xl border border-stone-200/80 overflow-hidden bg-stone-50/50"
                    >
                      <button
                        type="button"
                        onClick={() => setActiveDropdown(isOpen ? null : item.name)}
                        className={`w-full flex items-center justify-between p-3.5 font-bold text-left transition-colors ${
                          isOpen || isActive ? "text-[#830201] bg-red-50/60" : "text-[#2b3c6b]"
                        }`}
                      >
                            ...
                            return (
                              <Link
                                ...
                                className={`flex items-start gap-2.5 p-2.5 rounded-xl transition-colors ${
                                  isSubActive
                                    ? "bg-red-50 text-[#830201]"
                                    : "hover:bg-stone-50 text-stone-700"
                                }`}
                              >
                                <div className="w-7 h-7 rounded-lg bg-red-100/70 text-[#830201] flex items-center justify-center shrink-0 mt-0.5">
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setMobileOpen(false)}
                    className={`block px-4 py-3 rounded-xl font-bold text-[15px] transition-colors ${
                      isActive
                        ? "bg-[#830201] text-white"
                        : "text-[#2b3c6b] hover:bg-stone-100"
                    }`}
                  >
                    {item.name}
                  </Link>
            ...
            <div className="p-4 border-t border-stone-100 bg-stone-50/90 space-y-2.5">
              <a
                href="https://www.cambridgeuniforms.com"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-3 rounded-full bg-gradient-to-r from-[#830201] to-[#a30302] text-white font-bold text-xs uppercase tracking-wider shadow-md hover:shadow-lg transition-all"
              >
                <ShoppingBag className="w-4 h-4 text-amber-300" />
                <span>Order Cambridge Uniform</span>
              </a>

              <div className="grid grid-cols-2 gap-2 pt-1">
                <a
                  href="tel:7788227120"
                  className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-white border border-stone-200 text-[#2b3c6b] font-bold text-xs hover:border-[#830201] hover:text-[#830201] transition-colors text-center shadow-sm"
                >
                  <span>📞 Call Us</span>
                </a>
                <Link
